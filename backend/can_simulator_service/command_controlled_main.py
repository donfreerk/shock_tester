#!/usr/bin/env python3
"""
Command-Controlled CAN Simulator Service - Main Entry Point

Startet den CAN Simulator Service, der auf GUI-Kommandos reagiert
anstatt endlos zu laufen. Ersetzt den alten main.py für GUI-gesteuerte Tests.

Usage:
    python backend/can_simulator_service/command_controlled_main.py [options]
...
Beispiel:
    python backend/can_simulator_service/command_controlled_main.py --debug --log-level DEBUG
"""
...
try:
    ...
    from common.suspension_core.config import ConfigManager  # Korrekt: aus config package
...
class CommandControlledSimulatorService:
    ...
    def _start_test_from_command(self, message: Dict[str, Any]):
        ...
        logger.info(
            f"🚀 Test gestartet: {test_id} ({method} @ {position} für {duration}s)"
        )
    ...
    def _stop_current_test(self):
        ...
        if self.current_test and self.current_test.active:
            ...
            logger.info(f"⏹️ Test gestoppt: {self.current_test.test_id}")
    ...
    def _reset_simulator(self):
        ...
        logger.info("🔄 Simulator zurückgesetzt")
    ...
    def _publish_test_status(self, status: str):
        ...
        logger.debug(f"📊 Test-Status publiziert: {status}")
    ...
    def _publish_error_status(self, error_message: str):
        ...
        logger.error(f"❌ Fehler publiziert: {error_message}")

    async def start(self):
        """Startet den Service"""
        logger.info("🚀 Starte Command-Controlled Simulator Service...")
        self.running = True
        ...
        try:
            ...
            logger.info("✅ MQTT verbunden - warte auf Kommandos...")
            logger.info("📡 Abonnierte Topics:")
        ...
        while self.running:
            try:
                ...
                if not self.current_test and current_time - last_status_log > 30.0:
                    logger.info("⏳ Warte auf Test-Kommandos...")
                ...
                if self.current_test and self.current_test.active:
                    ...
                    if elapsed >= self.current_test.duration:
                        logger.info(
                            f"⏰ Test-Zeit abgelaufen: {self.current_test.test_id}"
                        )
                        self._stop_current_test()
                    ...
                    # Test-Daten generieren (falls Simulator aktiv)
                        ...
                        if data_point:
                            ...
                            if int(elapsed) % 3 == 0:
                                logger.info(
                                    f"📊 [{elapsed:5.1f}s] {data_point.frequency:5.1f}Hz, "
                                    f"{data_point.phase_shift:5.1f}°"
                                )
    ...
    async def shutdown(self):
        ...
        logger.info("🛑 Beende Command-Controlled Simulator Service...")
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Command-Controlled CAN Simulator Service",
...
async def main():
    ...
    try:
        ...
        print("🎛️ Wartet auf Test-Kommandos von der GUI...")
        ...
        print(
            '     "position": "front_right", "duration": 30, "method": "phase_shift"}}'
        )
        print("=" * 60)
