#!/usr/bin/env python3
"""
Hardware Bridge für Fahrwerkstester
...
class HardwareBridge:
    """
    Hardware Bridge mit Simulator-Unterstützung
    ...
    def __init__(self, config_path: Optional[str] = None):
        """
        Initialisiert die Hardware Bridge
        ...
        self.auto_save_interval = self.config.get(
            ["bridge", "auto_save_interval"], 30.0
        )
        ...
        logger.info(f"Hardware Bridge initialisiert - Modus: {self.bridge_mode.value}")

    async def _publish_mqtt(self, topic: str, payload: Dict[str, Any]) -> bool:
        """
        Universelle MQTT-Publish-Methode für sync/async Handler
        ...
        """
        try:
            if hasattr(self.mqtt_handler, 'publish'):
                # Synchroner MqttHandler
                return self.mqtt_handler.publish(topic, payload)
            else:
                # Async SimplifiedMqttClient
                await self.mqtt_handler.publish_async(topic, payload)
                return True
        except Exception as e:
            logger.error(f"MQTT-Publish-Fehler für {topic}: {e}")
            return False

    async def _disconnect_mqtt(self):
        """Universelle MQTT-Disconnect-Methode"""
        try:
            if hasattr(self.mqtt_handler, 'disconnect'):
                # Synchroner MqttHandler
            ...
            logger.error(f"MQTT-Disconnect-Fehler: {e}")
    ...
    def _load_fallback_config(self, config_path: Optional[str]) -> Dict[str, Any]:
        ...
        default_config = {
            "hardware_bridge": {"mode": "simulator"},
            ...
            "can": {"interface": "vcan0", "baudrate": 1000000, "protocol": "eusama"},
        }
        ...
        return type(
            ...
            (),
            {
                "get": lambda self, path, default=None: self._get_nested(
                    default_config, path, default
                ),
                "_get_nested": lambda self, d, path, default: d.get(path[0], default)
                if len(path) == 1
                else self._get_nested(d.get(path[0], {}), path[1:], default)
                if path[0] in d
                else default,
            },
        )()
    ...
    async def start(self):
        """Startet die Hardware Bridge"""
        logger.info("Starte Hardware Bridge...")
        ...
        try:
            ...
            logger.info("Hardware Bridge gestartet")
            return True

        except Exception as e:
            logger.error(f"Fehler beim Starten der Hardware Bridge: {e}")
            return False

    async def stop(self):
        """Stoppt die Hardware Bridge"""
        logger.info("Stoppe Hardware Bridge...")
        self.running = False
        ...
        logger.info("Hardware Bridge gestoppt")
    ...
    async def _init_can_interfaces(self) -> bool:
        ...
        try:
            ...
            if self.bridge_mode in [BridgeMode.SIMULATOR, BridgeMode.HYBRID]:
                ...
                self.simulator_can_interface = type(
                    "MockCanInterface",
                    (),
                    {
            ...
            if self.bridge_mode in [BridgeMode.HARDWARE, BridgeMode.HYBRID]:
                if SUSPENSION_CORE_AVAILABLE:
                    try:
                        ...
                        if self.can_interface:
                            self.can_interface.set_message_callback(
                                self._handle_hardware_can_message
                            )
                            logger.info(
                                "Hardware CAN-Interface erfolgreich initialisiert"
                            )
                        else:
                    ...
                    logger.info(
                        "Suspension Core nicht verfügbar - Hardware-Modus nicht möglich"
                    )
    ...
    async def _connect_mqtt(self) -> bool:
        ...
        """
        try:
            # MqttHandler ist synchron - verwende entsprechende Methoden
            if hasattr(self.mqtt_handler, 'connect'):
                # Synchroner MqttHandler
                success = self.mqtt_handler.connect()
                if not success:
                    logger.error("MqttHandler.connect() fehlgeschlagen")
                    return False
                
                # Synchrone Subscriptions (MqttHandler verwaltet Callbacks anders)
                # Verwende add_callback für Kategorien
                self.mqtt_handler.add_callback("commands", self._handle_bridge_command_sync)
                
                logger.info("MQTT-Verbindung und Callbacks erfolgreich eingerichtet")
                return True
                
            else:
                # Fallback für SimplifiedMqttClient (async)
                success = await self.mqtt_handler.connect_async()
                if not success:
                    logger.error("SimplifiedMqttClient.connect_async() fehlgeschlagen")
                    return False
                
                # Async Subscriptions für SimplifiedMqttClient
                await self.mqtt_handler.subscribe_async(
                    "suspension/bridge/command", self._handle_bridge_command
                )
                await self.mqtt_handler.subscribe_async(
                    "suspension/test/start", self._handle_test_start_command
                )
                await self.mqtt_handler.subscribe_async(
                    "suspension/test/stop", self._handle_test_stop_command
                )
    ...
    def _init_protocol(self):
        ...
        if SUSPENSION_CORE_AVAILABLE:
            try:
                ...
                # Verwende Mock-CAN-Interface für Protokoll im Simulator-Modus
                mock_can_interface = type('MockCanInterface', (), {})()
                self.protocol = create_protocol(protocol_name, mock_can_interface)
                if SUSPENSION_CORE_AVAILABLE:
                ...
                # Fallback auf Mock-Protokoll
                self._create_mock_protocol()
        else:
            logger.info("Mock-Protokoll für Fallback-Modus")
            self._create_mock_protocol()
    
    def _create_mock_protocol(self):
        """Erstellt Mock-Protokoll als Fallback"""
        self.protocol = type(
            "MockProtocol",
            (),
            {
    ...
    def _handle_hardware_can_message(self, message):
        ...
        try:
            ...
            logger.debug(
                f"Hardware CAN-Message empfangen: ID=0x{message_data['arbitration_id']:X}"
            )
    ...
    def _handle_simulator_can_message(self, message):
        ...
        try:
            ...
            logger.debug(
                f"Simulator CAN-Message empfangen: ID=0x{message_data['arbitration_id']:X}"
            )

        except Exception as e:
            logger.error(f"Fehler bei Simulator CAN-Message-Verarbeitung: {e}")

    def _handle_bridge_command_sync(self, message: Dict[str, Any]):
        """
        Synchroner Bridge-Command-Handler für MqttHandler
        
        Args:
            message: Command-Message
        """
        try:
            # Führe async Handler in Event-Loop aus
            asyncio.create_task(self._handle_bridge_command("commands", message))
        except Exception as e:
            logger.error(f"Fehler bei synchronem Bridge-Command-Handler: {e}")
    ...
    async def _start_test_session(
        self, position: str, test_id: str, metadata: Dict[str, Any]
    ):
        """
    ...
    async def _complete_test_session(self, status: str = "completed"):
        ...
        with self.queue_lock:
            ...
            session_data = [
                msg for msg in self.message_queue if msg["timestamp"] >= session_start
            ]
    ...
    def _decode_can_message(
        ...
        try:
            ...
            decoded = self.protocol.decode_message(
                message_data["arbitration_id"], message_data["data"]
            )
            ...
            logger.debug(
                f"Dekodierungsfehler für Message ID 0x{message_data['arbitration_id']:X}: {e}"
            )
    ...
    async def _publish_complete_dataset(self):
        ...
        complete_dataset = {
            ...
            "statistics": {
                ...
                "message_rate": len(self.current_session.raw_data)
                / (self.current_session.end_time - self.current_session.start_time),
                "data_sources": list(
                    set(
                        msg.get("source", "unknown")
                        for msg in self.current_session.raw_data
                    )
                ),
            },
        }
        ...
        await self._publish_mqtt(
            "suspension/raw_data/complete", complete_dataset
        )
        ...
        logger.info(
            f"Komplettes Dataset publiziert: {len(self.current_session.raw_data)} Datenpunkte"
        )
    ...
    async def _simulate_can_messages(self):
        ...
        if self.current_session and self.current_session.status == "running":
            ...
            force = (
                500
                + 100 * math.sin(2 * math.pi * freq * elapsed + phase_shift)
                + 10 * (0.5 - time.time() % 1)
            )
        ...
        while self.running:
            try:
                ...
                if (
                    ...
                    and time.time() - self.current_session.start_time
    ...
    async def _send_heartbeat(self):
        ...
        heartbeat_data = {
            ...
            "statistics": {
                ...
                "active_session": self.current_session.session_id
                if self.current_session
                else None,
            },
            "interfaces": {
        ...
        await self._publish_mqtt(
            "suspension/system/heartbeat", heartbeat_data
        )
    ...
    async def _auto_save_session_data(self):
        ...
        session_data = {
            ...
            "data_points": len(self.current_session.raw_data)
            if self.current_session.raw_data
            else 0,
        }

        await self._publish_mqtt(
            "suspension/test/session_update", session_data
        )
    ...
    async def _send_detailed_status(self):
        ...
        await self._publish_mqtt(
            "suspension/bridge/detailed_status", status_report
        )
    ...
    async def _set_bridge_mode(self, mode: str):
        ...
        try:
            ...
            if new_mode != self.bridge_mode:
                ...
                logger.info(
                    f"Bridge-Modus geändert: {old_mode.value} → {new_mode.value}"
                )
    ...
    async def _clear_message_buffer(self):
        ...
        await self._publish_mqtt(
            ...
            {"cleared_messages": cleared_count, "timestamp": time.time()},
        )
    ...
    async def _save_current_session(self):
        ...
        await self._publish_mqtt(
            ...
            {"session_data": session_data, "timestamp": time.time()},
        )
...
async def main():
    """Hauptfunktion für Hardware Bridge"""
    ...
    parser = argparse.ArgumentParser(description="Hardware Bridge für Fahrwerkstester")
    ...
    bridge = HardwareBridge(args.config)
